...
In a previous experiment using a similar dual-task paradigm, we showed the effect that increased working memory load has on the N2pc, Ptc, and SPCN components [@henare_working_2018]. In that task, participants were cued to encode either one or two colored shapes at the start of the trial and performed a set of visual searches during the maintenance period of the working memory task. The results showed that the Ptc elicited by distractors was reduced when working memory load was increased suggesting that increased working memory load impacts on an individual's ability to disengage from distractor objects. We showed no evidence of an effect of working memory load on target N2pc and therefore the result we observe in our current task is unlikely to be attributable to unnecessary storage of the irrelevant object in the working memory distractor present trials. If participants in the current task were simply storing the working memory distractor unnecessarily, then their responses should map onto the results we saw in our previous experiment. However, rather than seeing any effect of working memory distractors on Ptc amplitude, our current results show that working memory distractors affect target processing. This suggests that the effects we see in our current study are the result of specific mechanisms involved in the filtering of working memory distractors.

The fact that target N2pc amplitude increased in the presence of a working memory distractor may be somewhat counter-intuitive. Historically increased N2pc amplitude has been interpreted as representing a more efficient allocation of attention to an object. Increased N2pc amplitude is elicited by threatening stimuli [@weymar_specific_2013], highly rewarding stimuli [@kiss_reward_2009], and stimuli whose processing has been facilitated by intertrial priming [@tollner2008electrophysiological]. Therefore if the presence of a working memory distractor leads to attentional impairments, as suggested by our behavioural results, then we would have intuitively expected to see a concomitant decrease in N2pc amplitude. The observation instead that the presence of a working memory distractor leads to impaired behavioural performance but larger N2pc amplitudes, requires further investigation.

The relationship between N2pc amplitude and behavior, however, is not straight forward. While the studies mentioned above have typically shown improved behaviour coinciding with larger N2pc amplitude, a number of other studies have shown a mixed, or inverted relationship. @mazza2009attention for example showed across 3 experiments, a lack of any systematic relationship between N2pc amplitude and behavior. While response times were affected by factors such as target color variability, target-distractor distance, and distractor heterogeneity, N2pc was modulated only by distractor number. Furthermore, in their study of multiple object individuation, @mazza2013multiple demonstrate that increasing the number of targets in a display is associated with increased N2pc amplitude but slowed response times. These results highlight the complicated interaction between N2pc amplitude and performance, and suggest that modulation of N2pc amplitude is the result of many interacting factors.

One possible mechanism which could explain modulation of the target N2pc in our task is the effect that working memory representations have been shown to have on the guidance of attention. Previous research using dual tasks similar to the one we used have shown that the contents of memory can automatically guide attention in the secondary visual search task. When colored shapes are held in working memory, subsequent visual search performance is improved when the visual search target is placed within an object matching the stored shape [@kumar_electrophysiological_2009]. Similarly when participants are holding a particular color in working memory, distractors of that color in a subsequent visual search task cause more performance impairments than distractors of a different color [@olivers_feature-based_2006]. These results suggest that attention is automatically directed to the location of objects which match the contents of working memory, and it has also been shown that visual search targets that match working memory contents elicit larger N2pc components [@kumar_electrophysiological_2009]. 

In our task, the working memory objects and the visual search items were all either green or red, therefore the contents of working memory may have influenced our N2pc amplitude. In both the working memory distractor present and working memory distractor absent conditions, only one object was supposed to be stored and therefore this object matched the color of the visual search targets on 50% of trials. In working memory distractor present trials, however, there is a possibility that participants were encoding both objects on at least some portion of trials. This idea is supported by previous literature which used EEG evidence to show that irrelevant objects are often unnecessarily encoded into working memory [@vogel_neural_2004]. In our case, this would mean that both colors are stored in working memory and therefore the visual search target will match working memory contents on more than 50% of trials. The increased amplitude of the N2pc to targets in the working memory distractor present condition may be a result of the increased proportion of trials where the contents of working memory guide attention to matching visual search targets.
...
This suggestion would be consistent with previous findings showing that increased selection requirements lead to both slower repsonse times as well as an increase in N2pc amplitude [@mazza2011temporal; @mazza2013multiple; @naughtin2016early]. @mazza2011temporal showed that increasing the number of targets in a search display reuslts in slower response times and a larger N2pc, and this effect has also been replicated by @mazza2013multiple and @naughtin2016early. In that case, if the working memory distractor gets stored on some portion of trials and in turn causes the visual search distractor to be treated more like a target, this condition may be processed more similarly to the multiple target conditions in their studies. The working memory distractor-present condition therefore reflects a condition where selection requirements are increased during the visual search task, and therefore repsonse times are slowed while N2pc amplitude is increased. While this hypothesis is plausible and consistent with previous evidence, future research is required to establish whether it explains the results observed in our study.

An alternate account could be related to the effects of competitive interactions between a spatially bound working memory item, and a spatially located visual search target. A large body of evidence has accumulated to show that when two objects fall into the receptive field of a neuron, they compete for control of that neuron's firing via mutual suppression. N2pc has been shown to index this competitive suppression by @hilimire_competitive_2009 who showed that as objects in a visual display are moved closer together, N2pc amplitude is attenuated. Separate work in the working memory domain has shown that in order to maintain items in working memory, attention is directed to the location of the stored object. These two observations suggest that in our paradigm when the working memory item is being represented as a spatially bound set of features, the appearance of the visual search target could result in competition for representation. Therefore if the working memory distractor absent condition allows for more efficient storage of the item as a spatially bound object then we would see more of this competitive suppression represented as attenuation of N2pc amplitude.

Evidence suggests that attentional control is important for the binding of object features in working memory, and therefore the introduction of a working memory distractor could preoccupy attentional control and impair the binding process. @fougnie_attentive_2009 have shown that when an attention-demanding task is performed concurrently with a working memory task, memory for bound color-shape representations is impaired above and beyond the impairments observed for object features alone. Location also seems to play a central role in the encoding, maintenance, and retrieval of these bound representations [@treisman_location_2006]. It is plausible in our case that the presentation of an irrelevant distractor during working memory encoding engages attentional control mechanisms which would otherwise be involved in creating and maintaining a spatially located, bound representation of the target object. The difference between our working memory conditions would, therefore, represent differences in the extent to which the target object features are location bound, and N2pc attenuation in the distractor absent condition is the result of the visual search target competing for representation with this location bound working memory representation. 
...
\begin{table}[h]
	\centering
	\caption{Descriptive statistics for between study comparison of behavior}
	\label{tab:EpilogueBehav}
	\begin{tabular}{lrrrrrrrr}
		\hline
		\multicolumn{1}{c}{} & \multicolumn{2}{c}{WM-Acc} & \multicolumn{2}{c}{VS-Acc} & \multicolumn{2}{c}{WM-RT} & \multicolumn{2}{c}{VS-RT} \\
		\cline{2-3}\cline{4-5}\cline{6-7}\cline{8-9}
		 & 1 & 2 & 1 & 2 & 1 & 2 & 1 & 2  \\
		\hline
		Valid & 20 & 18 & 20 & 18 & 20 & 18 & 20 & 18  \\
		Missing & 0 & 0 & 0 & 0 & 0 & 0 & 0 & 0  \\
		Mean & 0.8725 & 0.8544 & 0.9170 & 0.9228 & 1244 & 931.2 & 741.0 & 532.1  \\
		Std. Deviation & 0.03810 & 0.05294 & 0.03614 & 0.07003 & 148.9 & 127.0 & 86.92 & 67.76  \\
		Minimum & 0.8200 & 0.7400 & 0.8400 & 0.6800 & 901.4 & 720.5 & 551.2 & 454.3  \\
		Maximum & 0.9500 & 0.9300 & 0.9700 & 0.9900 & 1583 & 1160 & 879.7 & 663.6  \\
		\hline
	\end{tabular} 
\end{table}

The design of this study is similar in many ways to the design of the previous study from chapter 3, however we observe some interesting differences between the results that we will explore further in the general discussion. In order to support the inferences we would like to make we have performed some statistical comparisons in order to test and quantify the observed differences. In \textit{Study Two}, the low load condition consisted of participants being shown two objects on a screen, while being expected to encode only one of those objects. In \textit{Study Three}, the distractor present condition also consisted of participants being shown two objects on screen and being expected to encode only one of those objects. While these two conditions are identical at the single trial level, they are embedded in different experimental contexts. Namely, cognitive load in \textit{Study Two} was much higher because partiicpants were storing either one or two objects on every trial whereas in \textit{Study Three} they only ever stored one object. \textit{Study One} Showed that these contextual differences can have significant effects on performance and therefore we tested whether participants would be significantly worse in \textit{Study Two} than \textit{Study Three}. We performed independent samples t-tests to compare performance between these two conditions on each behavioral measure and showed that while there was no significant effect of task context on accuracy in either the working memory task (t(36) = 1.22, p = 0.232) or the visual search task (t(36) = -0.32, p = 0.748), participants
in \textit{Study Two} were significantly slower in both the working memory task (t(36) = 6.93, p < 0.001) and visual search task (t(36) = 8.20, p < 0.001) than participants in \textit{Study Three}. See table\ \@ref(tab:EpilogueBehav) for a list of descriptive statistics for all of the above comparisons.

\begin{table}[h]
	\centering
	\caption{Descriptive Statistics for between study comparison of ERP components}
	\label{tab:EpilogueERP}
	\begin{tabular}{lrrrr}
		\hline
		\multicolumn{1}{c}{} & \multicolumn{2}{c}{Ptc-Diff} & \multicolumn{2}{c}{N2pc-Diff} \\
		\cline{2-3}\cline{4-5}
		 & 1 & 2 & 1 & 2  \\
		\hline
		Valid & 20 & 18 & 20 & 18  \\
		Missing & 0 & 0 & 0 & 0  \\
		Mean & 0.4735 & -0.03833 & 0.04450 & -0.3117  \\
		Std. Deviation & 0.8095 & 0.9317 & 0.7681 & 0.5588  \\
		Minimum & -1.520 & -1.990 & -1.700 & -1.530  \\
		Maximum & 1.390 & 1.620 & 1.450 & 0.5300  \\
		\hline
	\end{tabular} 
\end{table}

We also wanted to discuss the fact that Ptc amplitude was modulated by the manipulation in \textit{Study Two} but not in \textit{Study Three}. To test this effect directly we calculated the Ptc difference between conditions in \textit{Study Two}, and compared it to the difference between conditions in \textit{Study Three}. The results show that the difference is significantly larger in \textit{Study Two} than in \textit{Study Three} (t(36) = 1.81, p = 0.039) supporting the suggestion that there was a differential effect of study on Ptc modulation. Similarly, we wanted to discuss the possibility that N2pc was modulated by the manipulation in \textit{Study Three} but not in \textit{Study Two}. To test these effects directly we calculated the N2pc difference between conditions in \textit{Study Two}, and compared it to the difference between conditions in \textit{Study Three}. The results show that the difference is larger in \textit{Study Three} than in \textit{Study Two} however the p value is only approaching significance (t(36) = 1.62, p = 0.057). See table\ \@ref(tab:EpilogueERP) for a list of descriptive statistics for all of the above comparisons.
