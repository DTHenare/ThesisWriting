...
## Data processing
Trials which had no response were removed for all participants and performance measures were calculated for each block type and set size. For each of these conditions we calculated accuracy, hit rate, false alarm rate, capacity, D', and C.
non-responses were removed
calculate hits, fa capacity, cowans c , d' and C
groups created by...
...
Interaction between target number and dist number. For dist 0, t2<t4=t6. For dist 2, t2<t4=t6. For dist 4, t2<t4, t2=t6, t4=t6. For dist 6, t2<t4=t6. 
For targ2, no effects. For targ 4, 0>6, 0>6, no other effects. For targ 6, 0>4, 0>6, 2>4, 0=6  0=2,4=6. Figure\ \@ref(fig:capacitytargXdistPlot)
...
dev.off()
```
```{r sensitivity.fixedOnly.targets.table, results = 'asis'}
#apa_table(sensitivity.fixedOnly.targets.table$table, caption = "Fixed D'. Summary of the 3 x 4 repeated measures ANOVA used to test the effects of set size and distractor number on participant fa in the working memory task.", escape = FALSE)
```

```{r sensitivity_fixedOnly_pairwise, results = 'asis'}
#print(sensitivity.fixedOnly.targets.targmain.table, comment=FALSE, caption.placement = "top", latex.environments="flushleft")
```
...
```{r criterion.fixedOnly.targets.table, results = 'asis'}
#apa_table(criterion.fixedOnly.targets.table$table, caption = "Fixed criterion. Summary of the 3 x 4 repeated measures ANOVA used to test the effects of set size and distractor number on participant fa in the working memory task.", escape = FALSE)
```

```{r criterion_fixedOnly_pairwise, results = 'asis'}
#print(criterion.fixedOnly.targets.targmain.table, comment=FALSE, caption.placement = "top", latex.environments="flushleft")
```
...
## Variable distractor blocks

### Target matching block
#### Accuracy
...
#Difference between 0Dist conds and others
...
corPlot <- ggplot(data=fixed.Vals,aes(x=distDiff,y=avgCapacity)) +geom_point()
...
```{r measuringDistractorEffectInVariable}
#Difference between 0Dist conds and others
zeroDist <- subset(var.Vals, numDist==2 & DistNum=="targOpp")
nonZeroDist <- subset(var.Vals, numDist!=2 & DistNum=="targOpp")
zeroDist<- dplyr::summarise(group_by(zeroDist,PID),accuracy=mean(correctResponse))
nonZeroDist<- dplyr::summarise(group_by(nonZeroDist,PID),accuracy=mean(correctResponse))
for (subj in unique(allData$PID)) {
  var.Vals$distDiff[var.Vals$PID==subj] <- zeroDist$accuracy[zeroDist$PID==subj]-nonZeroDist$accuracy[nonZeroDist$PID==subj]
}
testCorrel <- dplyr::summarise(group_by(var.Vals,PID),cap = mean(avgCapacity), dist=mean(distDiff))
corValue <- cor(testCorrel$cap,testCorrel$dist)
corValuep <- cor.test(testCorrel$cap,testCorrel$dist)
corPlot <- ggplot(data=fixed.Vals,aes(x=distDiff,y=avgCapacity)) +geom_point()
...
```{r predictDistPresentWithDistAbsentAndSpan}
distPresent <- subset(fixed.Vals, DistNum==4|DistNum==6)
sheet <- summarise(group_by(distPresent,PID),distPPerf=mean(correctResponse),cap = mean(avgCapacity))
distAbsent <- subset(fixed.Vals, DistNum==0|DistNum==2)
sheet$distAPerf <- summarise(group_by(distAbsent,PID),distAPerf=mean(correctResponse))$distAPerf

model1 <- lm(distPPerf ~ distAPerf,sheet)
model2 <- lm(distPPerf ~ cap,sheet)
model3 <- lm(distPPerf ~ distAPerf+cap,sheet)
corPlot <- ggplot(data=fixed.Vals,aes(x=distDiff,y=avgCapacity)) +geom_point()
```
...
The number of irrelevant objects presented in the change detection task effects individual performance on the task.

There was no evidence of an effect of block context on individual performance.
...
r_refs(file = "WMBehav-references.bib")
```
