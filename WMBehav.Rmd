...
#Grab data for cronbachs and create correct responses (where "none is counted as incorrect")
cronbach <- allData
cronbach$key_resp_2.keys[cronbach$key_resp_2.keys=="None"] = ""
cronbach$key_resp_2.keys <- factor(cronbach$key_resp_2.keys)
cronbach$correctResponse <- as.integer(cronbach$CorrAns==cronbach$key_resp_2.keys)
cronbach <- cronbach[order(cronbach$targetColour),]
cronbach <- cronbach[order(cronbach$matchType),]
cronbach <- cronbach[order(cronbach$targetNum),]
cronbach <- cronbach[order(cronbach$DistNum),]
cronbach <- within(cronbach, rm(targetColour,DistNum,matchType,targetNum,CorrAns,key_resp_2.keys,key_resp_2.rt))
...
for (subj in 1:length(nonRespFails)) {
  ...
  cronbach <- cronbach[!cronbach$PID==nonRespFails[subj],]
}
}
...
for (subj in 1:length(accuracyFails)) {
  ...
  cronbach <- cronbach[!cronbach$PID==accuracyFails[subj],]
}
}
#check remaining
remSubj <- as.integer(levels(as.factor(cronbach$PID)))
length(summary(as.factor(allData$PID)))
#create cronbach shape
cronbachData <- as.data.frame(t(cronbach$correctResponse[cronbach$PID==1]))
cronbachData <- cronbachData[0,]
for (subj in remSubj) {
  curData <- as.data.frame(t(cronbach$correctResponse[cronbach$PID==remSubj]))
  cronbachData <- rbind(cronbachData,curData)
}
```
