...
Participants performed two practice blocks of .X. trials each, followed by 18 blocks of 40 trials each for a total of 720 experiment trials. For each block, the participant would be told which colour (either blue or red) they would be focusing on for that block. Trials then involved being presented with a set of stimuli, some of which matched the target colour, and some of which did not. Participants were required to encode only the subset of objects defined by the target colour, maintain that information across a short delay period, and were then presented with the encoding the display with one of the target objects 

At the start of each trial a cue was presented for 500ms followed by a 500ms interval. The encoding display was then shown for 250ms, followed by a 1 second delay period. The retrieval display was then presented for 2 seconds, or until the participant responded. A fixation cross was maintained in the centre of the screen throughout the entire trial.

The experiment was separated into 18 blocks of 40 trials each for a total of 720 trias per person. 

Each encoding display consisted of either 2, 4, or 6 targets, and either 0, 2, 4, or 6 distractors. 

# Results

## Reliability
```{r reliability,echo=FALSE,include=FALSE}
oddVeven.cor <- cor(oddTable$cowanC, evenTable$cowanC)
```
...
Interaction between target number and dist number. For dist 0, 2<4<6. For dist 2, 2<4<6. For dist 4, 2<4, 2<6, 4=5. For dist 6 2<4, 2<6, 4=6. For targ 2, no differences. For targ 4, 0>4,0>6, 0trending>2. For targ 6, 0>4, 0>6, 2>4. Figure\ \@ref(fig:accuracytargXdistPlot)
...
Interaction between target number and dist number. For dist 0, t2<t4=t6. For dist 2, t2<t4<t6. For dist 4, t2<t4, t2<t6, t4=t6. For dist 6, t2<t4, t2<t6, t4=t6. 
For targ2, no effects. For targ 4, 0>6 no other effects. For targ 6, 0>4, 0>6, 2>4, 2>6,  0=2,4=6. Figure\ \@ref(fig:capacitytargXdistPlot)
...
## Hits -issue with fixvmat results
### Fixed distractor blocks
...
Interaction between target number and dist number. For dist 0, t2>t6, t4>t6, t2trending>t4. for dit 2, 2>4>6. for dist 4, 2>4>6. for dist 6, 2>4>6. 
For t2, no dist diffs. For t4 0=2, 0>4, 0>6, 2=4, 2=6, 4=6. For dist 6, 0=2, 0>4, 0>6, 2>4, 2trending>6, 4=6. Figure\ \@ref(fig:hitstargXdistPlot)
...
### Fixed distractor vs variable distractor blocks
#### Fixed vs targEqual - interaction =0.059
...
Main effect of target number. 2<4<6. Figure\ \@ref(fig:fatargXdistPlot)
...
oddJack <- group_by(p1, PID, targetNum, DistNum)
oddJackTable <- dplyr::summarise(oddJack, cap=mean(jackCapacity))
evenJack <- group_by(p1, PID, targetNum, DistNum)
evenJackTable <- dplyr::summarise(evenJack, cap=mean(jackCapacity))

oddCheck <- dplyr::summarise(group_by(oddJackTable, PID),cap=mean(cap))
evenCheck <- dplyr::summarise(group_by(evenJackTable, PID),cap=mean(cap))
...
fixed.Vals$numOfDist <- as.integer(as.character(fixed.Vals$DistNum))
...
interactionData <- summarySE(fixed.Vals, measurevar="cowanC", groupvars=c("targetNum","DistNum","Group"))
interaction <- ggplot(interactionData, aes( x=DistNum,y=cowanC, color = Group)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=cowanC-se, ymax=cowanC+se), size=0.5,   
    width=.25,position=position_dodge(.9)) +
  facet_grid(~targetNum, labeller = as_labeller(c(`2`="Set size 2", `4`="Set size 4", `6`="Set size 6"))) +
...
lowGroupdata <- summarySE(lowGroup, measurevar="correctResponse", groupvars=c("targetNum","DistNum"))
lowGroupACC <- ggplot(lowGroupdata, aes( x=DistNum,y=correctResponse)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=correctResponse-se, ymax=correctResponse+se), size=0.5,   
    width=.25,position=position_dodge(.9)) +
  facet_grid(~targetNum, labeller = as_labeller(c(`2`="Set size 2", `4`="Set size 4", `6`="Set size 6"))) +
...
highGroupdata <- summarySE(highGroup, measurevar="correctResponse", groupvars=c("targetNum","DistNum"))
highGroupACC <- ggplot(highGroupdata, aes( x=DistNum,y=correctResponse)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=correctResponse-se, ymax=correctResponse+se), size=0.5,   
    width=.25,position=position_dodge(.9)) +
  facet_grid(~targetNum, labeller = as_labeller(c(`2`="Set size 2", `4`="Set size 4", `6`="Set size 6"))) +
