---
title             : "Working memory capacity in the change detection task is effected by the number of irrelevant objects in the display"
...
pFails <- numberOfTrials$PID[numberOfTrials$nrow<700]
#remove failed participants
for (subj in 1:length(pFails)) {
  allData <- allData[!allData$PID==pFails[subj],]
}
...
pFails <- easyAccuracy$PID[easyAccuracy$avg<0.60]
for (subj in 1:length(pFails)) {
  allData <- allData[!allData$PID==pFails[subj],]
}
#check remaining
...
### Fixed distractors
#### Targets
```{r capacity_fixedonly_targets, echo=FALSE}
...
#### Distractors
```{r fcapacity_fixedonly_distractors, echo=FALSE}
...
#### Targets
```{r capacity_variableonly_targets, echo=FALSE}
...
#Interaction
var.DISTbyBLOCK.capacity.posthoc <- pairs(emmeans(var.distractor.capacity.aov, ~ numDist|Block))
var.BLOCKbyDIST.capacity.posthoc <- pairs(emmeans(var.distractor.capacity.aov, ~ Block|numDist))
#Interaction table
summary(var.DISTbyBLOCK.capacity.posthoc)
summary(var.BLOCKbyDIST.capacity.posthoc)
```
#### Distractors
```{r capacity_variableonly_distractors, echo=FALSE}
...
```{r capacity_fixVmatch_distractors, echo=FALSE}
...
```{r capacity_fixVopp_distractors, echo=FALSE}
...
var.distractor.capacity.posthoc <- pairs(emmeans(fix_v_mat.capacity.aov, ~ numDist))
summary(var.distractor.capacity.posthoc)
