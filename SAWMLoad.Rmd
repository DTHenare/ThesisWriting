...
Working memory loads of one and two were chosen for two main reasons. First, there is a possibility that exhausting working memory capacity has a qualitatively different effect on processing than simply varying the amount available to an individual. We opted to take the more conservative approach and test whether neural indices could detect the titration of working memory availability rather than distinguish between two (potentially) different processing states. Second, while visual working memory is typically shown to be around 4 visual objects, initial piloting of this task showed that participants were struggling significantly with anything over two items. It was important to avoid having a working memory load where participants performed close to chance because this could also be interpreted as particpants not attending to and performing the working memory task at all.
...
While our results seem to show that working memory load impacts on visual search performance, these results are inconsistent with a set of studies which find no such effect [@woodman2001visual;@logan1978attention]. These studies investigated the role of working memory load on search efficiency using a dual task similar to the task used here. Search efficiency refers to a specific phenomenon where increasing the number of to-be-searched items in a display leads to a systematic slowing of repsonse times. In their studies, @logan1978attention and @woodman2001visual show that a concurrent working memory load will delay response times overall, however it will not lead to a decrease in the efficiency of responses as the visual search set size increases. A key distincttion between the study designs they both used and the study designed used here is that the visual search distractor in our task is a salient distractor. @lavie_load_2004, have suggested that working memory's role is specifically relevant in situations where targets are competing for attention with low priority but highly salient distractors. Our results provide support for this idea and suggest that this may be a significant factor which reconciles these apparently conflicting results.
