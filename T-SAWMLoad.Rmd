...
pdf("Figures/lowCorrelogram.pdf", width = 7, paper = "special")
corrplot(low.area.corr$r, method = "ellipse", type = "lower",p.mat = low.area.corr$P, sig.level = 0.05, insig = "blank")
dev.off()
pdf("Figures/highCorrelogram.pdf", width = 7, paper = "special")
corrplot(high.area.corr$r, method = "ellipse", type = "lower",p.mat = high.area.corr$P, sig.level = 0.05, insig = "blank")
dev.off()

# Scatterplots
VSRTbyDISTP <- ggplot(lowAreaData, aes(x=VSRT, y=DistP)) + geom_point() +geom_smooth(method=lm)  + scale_y_continuous(name =expression(paste("Distractor Positivity (",mu,"v*ms)"))) +
  scale_x_continuous("Visual Search RT (ms)") + theme_apa()
VSRTbyDISTN <- ggplot(lowAreaData, aes(x=VSRT, y=DistN)) + geom_point() +geom_smooth(method=lm) + scale_y_continuous(name =expression(paste("Distractor Negativity (",mu,"v*ms)"))) +
  scale_x_continuous("Visual Search RT (ms)")+theme_apa()
VSRTbyTARGP <- ggplot(lowAreaData, aes(x=VSRT, y=TargP)) + geom_point() +geom_smooth(method=lm) + scale_y_continuous(name =expression(paste("Target Positivity (",mu,"v*ms)"))) +
  scale_x_continuous("Visual Search RT (ms)")+theme_apa()
WMRTbyDISTP <- ggplot(lowAreaData, aes(x=WMRT, y=DistN)) + geom_point() +geom_smooth(method=lm) + scale_y_continuous(name =expression(paste("Distractor Positivity (",mu,"v*ms)"))) +
  scale_x_continuous("Working Memory RT (ms)")+theme_apa()
pdf("Figures/SAWMLOAD_lowScatterPlot.pdf", width = 7, paper = "special")
plot_grid(VSRTbyDISTP,VSRTbyDISTN,VSRTbyTARGP,WMRTbyDISTP, labels = c("A","B","C","D"))
dev.off()

VSACCbyTARGN  <- ggplot(highAreaData, aes(x=VSAcc, y=TargN)) + geom_point() +geom_smooth(method=lm) +scale_y_continuous(name =expression(paste("Target Negativity (",mu,"v*ms)"))) + scale_x_continuous("Visual Search Acc (%)") +theme_apa()
pdf("Figures/SAWMLOAD_highScatterPlot.pdf", width = 7, paper = "special")
VSACCbyTARGN
dev.off()
```
...
include_graphics("Figures/SAWMLOAD_lowScatterPlot.pdf", auto_pdf = TRUE)
```
...
include_graphics("Figures/SAWMLOAD_highScatterPlot.pdf", auto_pdf = TRUE)
```
...
include_graphics("Figures/lowCorrelogram.pdf", auto_pdf = TRUE)
```
...
include_graphics("Figures/highCorrelogram.pdf", auto_pdf = TRUE)
```
